...
- `src/git_recap/models.py` — Dataclass models + JSON/JSONL serialization utilities. Raw models: `FileChange`, `Comment`, `Review`, `PRRaw`, `CommitRaw`, `IssueRaw`. Normalized: `ActivityKind` (enum), `Activity`, `DailyStats`. Async job: `JobStatus` (enum), `Job`. LLM tracking: `TokenUsage` (supports `+` operator). Factory functions: `pr_raw_from_dict()`, `commit_raw_from_dict()`, `issue_raw_from_dict()`, `activity_from_dict()`, `daily_stats_from_dict()`. `Activity` preserves text context: `body` (PR/commit/issue body), `review_bodies`, `comment_bodies`.
- `src/git_recap/exceptions.py` — Exception hierarchy: `GitRecapError` base → `FetchError`, `NormalizeError`, `SummarizeError`, `StepFailedError`.
- `src/git_recap/logging_config.py` — `setup_logging(level)`: stderr handler, `HH:MM:SS LEVEL [module] message` format, silences noisy third-party loggers (httpx, openai, anthropic). Idempotent. `reset_logging()` for tests.
...
- `src/git_recap/infra/ghes_client.py` — HTTP client with retry (429 + 403 rate limit + 5xx), search API throttle (`search_interval=2.0s`), pagination, adaptive rate limiting (warns <100, waits <10 remaining). Thread-safe: `_throttle_search()` protected by `threading.Lock`, rate limit state by separate lock. `search_interval` kwarg controls delay between Search API calls (30 req/min limit). Context manager support.
- `src/git_recap/infra/llm_client.py` — `LLMClient`: provider-agnostic (OpenAI/Anthropic), `timeout=120s`, `max_retries=3` passed to SDK constructors. `chat(system_prompt, user_content) → str`. Thread-safe token usage tracking via `_usage_lock`. Logs provider, model, token counts, elapsed time.
...
- `src/git_recap/api/app.py` — FastAPI app. Routes in `api/routes/`. `StaticFiles` mount for `frontend/`. Registers routers: pipeline, fetch, normalize, summarize_pipeline, summary, query. CORS middleware.
- `src/git_recap/api/deps.py` — FastAPI dependency injection: `get_config()` (lru_cache), `get_job_store()`.
- `src/git_recap/api/job_store.py` — `JobStore`: async job CRUD via `data/state/jobs/{job_id}.json`. `create()`, `get(job_id)`, `update(job_id, status, result, error)`.
- `src/git_recap/api/routes/pipeline.py` — Full pipeline endpoints. `POST /api/pipeline/run/{date}` (optional body: force, types, enrich), `POST /api/pipeline/run/range` (body: since, until, force, types, max_workers, enrich, summarize_weekly/monthly/yearly). `GET /api/pipeline/jobs/{id}`. All async via BackgroundTasks (202 + job_id). Hierarchical summarization via `_run_hierarchical` helper when `summarize_weekly/monthly/yearly` set. Injects `FetchProgressStore`, `DailyStateStore`, `GHESClientPool` (when workers>1). `ghes.close()` and `pool.close()` in finally.
- `src/git_recap/api/routes/fetch.py` — Individual fetch endpoints. `POST /api/pipeline/fetch/{date}` (optional body: types, force), `POST /api/pipeline/fetch/range` (body: since, until, types, force, max_workers). `GHESClientPool` when workers>1. Resource cleanup in finally.
- `src/git_recap/api/routes/normalize.py` — Individual normalize endpoints. `POST /api/pipeline/normalize/{date}` (optional body: enrich, force), `POST /api/pipeline/normalize/range` (body: since, until, force, enrich, max_workers). `enrich=False` skips LLM creation.
- `src/git_recap/api/routes/summarize_pipeline.py` — Summarize trigger endpoints. `POST /api/pipeline/summarize/daily/{date}`, `POST /api/pipeline/summarize/daily/range` (body: since, until, force, max_workers), `POST /api/pipeline/summarize/weekly` (body: year, week, force), `POST /api/pipeline/summarize/monthly` (body: year, month, force), `POST /api/pipeline/summarize/yearly` (body: year, force).
- `src/git_recap/api/routes/summary.py` — Read-only summary endpoints. `GET /api/summary/daily/{date}`, `GET /api/summary/weekly/{year}/{week}`, `GET /api/summary/monthly/{year}/{month}`, `GET /api/summary/yearly/{year}`.
- `src/git_recap/api/routes/query.py` — Free-form query endpoint. `POST /api/query` (body: question, months).
...
- **Exception hierarchy:** See `src/git_recap/exceptions.py`.
