...
- `src/workrecap/scheduler/notifier.py` — `Notifier` ABC + `LogNotifier`, `TelegramNotifier` (httpx POST to Bot API, formats event as text message), `CompositeNotifier` (fans out to multiple notifiers, individual failures isolated). `SchedulerEvent` dataclass (job, status, triggered_at, target, completed_at, error).
