...
- `src/workrecap/scheduler/config.py` — `ScheduleConfig`: parses `schedule.toml` (project root). Dataclass hierarchy: `DailySchedule` (time/enrich/batch/workers), `WeeklySchedule` (day/time), `MonthlySchedule` (day/time), `YearlySchedule` (month/day/time), `NotificationConfig` (on_failure/on_success). `from_toml(path)` classmethod, returns defaults when file missing. `hour`/`minute` properties parse `"HH:MM"` time strings.
...
- `src/workrecap/scheduler/notifier.py` — `Notifier` ABC + `LogNotifier`. `SchedulerEvent` dataclass (job, status, triggered_at, target, completed_at, error). Extensible for Telegram, system notifications.
