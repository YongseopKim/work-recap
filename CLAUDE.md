...
- `src/workrecap/infra/llm_router.py` — `LLMRouter`: drop-in replacement for `LLMClient`. Task-based provider+model routing via `chat(system, user, task="daily")`. Strategy modes: economy, standard, premium, adaptive, fixed. Lazy provider creation with double-checked locking. `usage` property (backward compat) + `usage_tracker` property (per-model breakdown). Integrates `EscalationHandler` for adaptive mode. `chat()` accepts `json_mode`, `max_tokens`, `cache_system_prompt` kwargs; `cache_system_prompt` defaults to `True` (Anthropic: ephemeral cache_control, OpenAI: auto-caching, flag ignored); `max_tokens` resolution: explicit kwarg > task config (config.toml) > None (bound to output format, not model — same value on escalation). **Batch methods:** `submit_batch(requests, task)`, `get_batch_status(batch_id, task)`, `get_batch_results(batch_id, task)`, `wait_for_batch(batch_id, task, timeout, poll_interval, batch_size, progress)`. **Dynamic batch timeout:** `batch_size` auto-computes timeout (base 5min + 30s/request, cap 4h); `poll_interval=None` uses adaptive polling (5s→60s linear ramp). Batch uses base_model only (no escalation).
- `src/workrecap/infra/providers/` — Provider abstraction layer. `base.py`: `LLMProvider` ABC + `ModelInfo` dataclass. `chat()` signature: `json_mode` (structured JSON output), `max_tokens` (output limit), `cache_system_prompt` (enable prompt caching). `batch_mixin.py`: `BatchRequest`, `BatchResult`, `BatchStatus`, `BatchCapable` ABC (mixin for providers supporting batch). Concrete: `openai_provider.py` (accepts optional `base_url` for proxy routing; uses `response_format`, maps `max_tokens` → `max_completion_tokens` for SDK compatibility; reasoning models (gpt-5/o3/o4) skip `max_completion_tokens` since it includes thinking tokens — `_REASONING_PREFIXES` list, `_is_reasoning_model()` check; extracts `cached_tokens` from usage; `BatchCapable` via JSONL file upload), `anthropic_provider.py` (accepts optional `base_url` for proxy routing; JSON via assistant prefill `[`, caching via `cache_control: {"type": "ephemeral"}`, extracts `cache_read/creation_input_tokens`; `BatchCapable` via `client.messages.batches`), `custom_provider.py` (OpenAI SDK + base_url for Ollama/vLLM, no batch support).
- `src/workrecap/infra/provider_config.py` — `ProviderConfig`: parses `.provider/config.toml` (TOML, stdlib `tomllib`) for multi-provider task routing. Requires config.toml to exist (no fallback). `TaskConfig` (includes `max_tokens: int | None`), `ProviderEntry` (includes `base_url: str | None`) dataclasses. `validate()` checks consistency; skips api_key check for providers with `base_url` set (proxy mode).
...
- `src/workrecap/api/app.py` — FastAPI app with `lifespan` async context manager. Lifespan creates `SchedulerService` from `schedule.toml` with `CompositeNotifier` (LogNotifier + optional TelegramNotifier when `telegram.enabled` and `AppConfig` has bot_token/chat_id), stores on `app.state.scheduler` and `app.state.scheduler_history`. Graceful degradation: scheduler init failure → disabled mode. Routes in `api/routes/`. `StaticFiles` mount for `frontend/`. Registers routers: pipeline, fetch, normalize, summarize_pipeline, summary, summaries_available, query, scheduler. CORS middleware.
...
- `src/workrecap/api/job_store.py` — `JobStore`: async job CRUD via `data/state/jobs/{job_id}.json`. `create()`, `get(job_id)`, `update(job_id, status, result, error)`, `update_progress(job_id, progress)` (updates progress text without changing status).
- `src/workrecap/api/routes/pipeline.py` — Full pipeline endpoints. `POST /api/pipeline/run/{date}` (optional body: force, types, enrich), `POST /api/pipeline/run/range` (body: since, until, force, types, max_workers, enrich, batch, summarize_weekly/monthly/yearly). `GET /api/pipeline/jobs/{id}` (includes `progress` field). `GET /api/pipeline/jobs/{id}/stream` — **SSE endpoint**: streams job status updates via `text/event-stream`; polls job file every 0.5s, emits event only when `updated_at` changes, auto-closes on terminal status. All async via BackgroundTasks (202 + job_id). Range pipeline reports date-level progress (`"1/5"`, `"2/5"`, ...) via `update_progress()`. Hierarchical summarization via `_run_hierarchical` helper when `summarize_weekly/monthly/yearly` set. Injects `FetchProgressStore`, `DailyStateStore`, `GHESClientPool` (when workers>1). `ghes.close()` and `pool.close()` in finally.
...
- `src/workrecap/api/routes/scheduler.py` — Scheduler endpoints. `GET /api/scheduler/status` (state + job list), `GET /api/scheduler/history` (execution history, `?job=`/`?limit=` filters), `POST /api/scheduler/trigger/{job}` (manual trigger, 202), `PUT /api/scheduler/pause`, `PUT /api/scheduler/resume`. Routes access scheduler via `request.app.state.scheduler` (set by lifespan).
- `src/workrecap/scheduler/config.py` — `ScheduleConfig`: parses `schedule.toml` (project root). Dataclass hierarchy: `DailySchedule` (time/enrich/batch/workers), `WeeklySchedule` (day/time), `MonthlySchedule` (day/time), `YearlySchedule` (month/day/time), `NotificationConfig` (on_failure/on_success), `TelegramConfig` (enabled). `from_toml(path)` classmethod, returns defaults when file missing. `hour`/`minute` properties parse `"HH:MM"` time strings.
- `src/workrecap/scheduler/core.py` — `SchedulerService`: APScheduler `AsyncIOScheduler` wrapper. `start()` registers CronTrigger jobs (daily/weekly/monthly/yearly). `shutdown()`, `pause()`, `resume()`. `status()` returns `{"state": "running"|"paused"|"stopped"|"disabled", "jobs": [...]}`. `get_jobs()` returns job id + next_run_time. FastAPI lifespan manages lifecycle via `app.state.scheduler`.
- `src/workrecap/scheduler/jobs.py` — Async job functions: `run_daily_job` (yesterday fetch→normalize→summarize via `_build_orchestrator`), `run_weekly_job` (last week summary via `_build_summarizer`), `run_monthly_job` (last month weekly→monthly cascade), `run_yearly_job` (last year weekly→monthly→yearly cascade). Each records to `SchedulerHistory` and notifies via `Notifier`. `SummarizeError` from intermediate cascade steps is swallowed. **Telegram summary:** All 4 jobs call `summarizer.telegram_summary()` after successful pipeline — graceful failure (warning log only, never breaks notification).
- `src/workrecap/scheduler/history.py` — `SchedulerHistory`: JSON file (`data/state/scheduler_history.json`). Thread-safe (`threading.Lock`). `record(event)`, `list(job?, limit?)`. Max 100 entries (FIFO). Persistence across instances.
- `src/workrecap/scheduler/notifier.py` — `Notifier` ABC + `LogNotifier`, `TelegramNotifier` (httpx POST to Bot API, reads `.telegram.txt` for message body), `CompositeNotifier` (fans out to multiple notifiers, individual failures isolated). `SchedulerEvent` dataclass (job, status, triggered_at, target, completed_at, error). `TelegramNotifier` reads pre-generated `.telegram.txt` via `_read_summary()` + `_resolve_telegram_path()`, combines with header via `_build_message()`. No regex formatting — LLM handles all text formatting in summarizer.
- `schedule.toml` — Scheduler configuration: `[scheduler]` enabled/timezone, `[scheduler.daily]` time/enrich/batch/workers, `[scheduler.weekly]` day/time, `[scheduler.monthly]` day/time, `[scheduler.yearly]` month/day/time, `[scheduler.notification]` on_failure/on_success, `[scheduler.telegram]` enabled (default false).
...
└── js/
    ...
    ├── api.js          ← fetch helpers: api(), pollJob(), streamJob(SSE), escapeHtml(), copyToClipboard(), getISOWeek(), getISOWeekYear()
    ...
    └── scheduler.js    ← Scheduler tab: status, jobs, history, manual trigger, Telegram notification config
```

**4 tabs:** Pipeline, Summaries (calendar view), Ask (chat history), Scheduler (auto-scheduling). Dark mode toggle via `data-theme` + `localStorage`.
...
data/summaries/{YYYY}/weekly/       → W{NN}.md, W{NN}.telegram.txt
data/summaries/{YYYY}/monthly/      → {MM}.md, {MM}.telegram.txt
data/summaries/{YYYY}/              → yearly.md, yearly.telegram.txt
data/state/                         → checkpoints.json (last_fetch/normalize/summarize_date), daily_state.json (per-date timestamps), failed_dates.json (failed date tracking for auto-retry), fetch_progress/ (chunk search cache), jobs/, scheduler_history.json
```
...
- **LLM Proxy support:** OpenAI/Anthropic providers accept optional `base_url` in config.toml for proxy server routing. Proxy mode: `base_url` set + `api_key = "unused"` (proxy manages keys). Validation skips api_key check when `base_url` is set. `LLMRouter._create_provider()` passes `base_url` to provider constructors. Provider SDKs handle all routing transparently — no request/response format changes.
- **Provider task routing:** Services pass `task=` kwarg to `LLMRouter.chat()`: normalizer uses `task="enrich"`, summarizer uses `task="daily"/"weekly"/"monthly"/"yearly"/"query"/"telegram"`. Router maps each task to its configured provider+model. `telegram` task uses lightweight model (haiku) for cost efficiency.
- **LLM API optimization features:** `json_mode=True` for structured output (provider-native JSON constraints), `max_tokens` for per-task output limits (config.toml or explicit, bound to output format not model — same value on escalation; OpenAI provider maps to `max_completion_tokens` for SDK compatibility; **reasoning models** (gpt-5/o3/o4) skip `max_completion_tokens` entirely since it includes thinking tokens and a low limit starves visible output), `cache_system_prompt` defaults to `True` in `LLMRouter.chat()` — Anthropic uses `cache_control: {"type": "ephemeral"}`, OpenAI uses automatic implicit caching (flag ignored).
- **Prompt template split convention:** Templates use `<!-- SPLIT -->` marker to separate static instructions (→ system prompt, cacheable) from dynamic data (→ user content). `_render_split_prompt()` in summarizer, manual split in normalizer. `prompts/telegram.md`: shared template for all 4 levels (daily/weekly/monthly/yearly) — 4000-char plain text, proportional repo allocation, topic-based grouping, emoji headings.
