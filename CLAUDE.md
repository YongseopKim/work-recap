...
- `src/workrecap/config.py` — `AppConfig` (pydantic-settings), derives all data paths. Env vars from `.env` (GHES connection + file paths only; LLM settings live in `.provider/config.toml`). `extra="ignore"` so leftover `LLM_*` vars in `.env` are harmless. `max_workers: int = 5` controls parallel LLM execution (override via `MAX_WORKERS` env var). `max_fetch_retries: int = 5` controls how many times a failed date is retried before marking it exhausted (override via `MAX_FETCH_RETRIES` env var). `enabled_sources: list[str] = ["github"]` for multi-source support. `provider_config_path` property returns `Path(".provider/config.toml")`.
...
- **LLM API optimization features:** `json_mode=True` for structured output (provider-native JSON constraints), `max_tokens` for per-task output limits (config.toml or explicit, bound to output format not model — same value on escalation; OpenAI provider maps to `max_completion_tokens` for SDK compatibility with gpt-5/o3+), `cache_system_prompt` defaults to `True` in `LLMRouter.chat()` — Anthropic uses `cache_control: {"type": "ephemeral"}`, OpenAI/Gemini use automatic implicit caching (flag ignored).
