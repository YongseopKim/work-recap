...
- `src/workrecap/config.py` — `AppConfig` (pydantic-settings), derives all data paths. Env vars from `.env`. `max_workers: int = 5` controls parallel LLM execution (override via `MAX_WORKERS` env var).
...
- `src/workrecap/logging_config.py` — `setup_logging(level)`: stderr handler, `HH:MM:SS LEVEL [module] message` format, silences noisy third-party loggers (httpx, openai, anthropic). Idempotent. `reset_logging()` for tests.
...
- `src/workrecap/infra/llm_client.py` — `LLMClient`: provider-agnostic (OpenAI/Anthropic), `timeout=120s`, `max_retries=3` passed to SDK constructors. `chat(system_prompt, user_content) → str`. Thread-safe token usage tracking via `_usage_lock`. Logs provider, model, token counts, elapsed time.
...
- `src/workrecap/api/deps.py` — FastAPI dependency injection: `get_config()` (lru_cache), `get_job_store()`.
