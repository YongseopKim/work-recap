...
- `src/workrecap/config.py` — `AppConfig` (pydantic-settings), derives all data paths. Env vars from `.env` (GHES connection + file paths only; LLM settings live in `.provider/config.toml`). `extra="ignore"` so leftover `LLM_*` vars in `.env` are harmless. `max_workers: int = 5` controls parallel LLM execution (override via `MAX_WORKERS` env var). `provider_config_path` property returns `Path(".provider/config.toml")`.
