...
- **LLM API optimization features:** `json_mode=True` for structured output (provider-native JSON constraints), `max_tokens` for per-task output limits (config.toml or explicit), `cache_system_prompt=True` for Anthropic prompt caching (others auto-cache or ignore).
