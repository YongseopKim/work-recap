...
# Run unit tests (594 tests, integration excluded by default)
pytest
...
- `src/git_recap/services/daily_state.py` — `DailyStateStore`: per-date timestamp tracking for fetch/normalize/summarize. Thread-safe (`threading.Lock`). Staleness rules: fetch stale if `fetched_at.date() <= target_date`; normalize stale if `fetch_ts > normalize_ts` (cascade); summarize stale if `normalize_ts > summarize_ts` (cascade). Persists to `data/state/daily_state.json`.
...
- `src/git_recap/cli/main.py` — Typer app. Subcommands: `fetch`, `normalize`, `summarize`, `run`, `ask`. All four commands support checkpoint catch-up and `--weekly/--monthly/--yearly` options. All four commands support `--force/-f`. `summarize weekly/monthly/yearly` also support `--force/-f` (skip-if-exists). `fetch` and `run` support `--type/-t` for type filtering. `normalize`, `summarize daily`, `run` support `--workers/-w` (default: `config.max_workers=5`). `normalize` and `run` support `--enrich/--no-enrich` (default: `True`). All inject `DailyStateStore` into services.
