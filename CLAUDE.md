...
- **LLM API optimization features:** `json_mode=True` for structured output (provider-native JSON constraints), `max_tokens` for per-task output limits (config.toml or explicit, bound to output format not model — same value on escalation), `cache_system_prompt` defaults to `True` in `LLMRouter.chat()` — Anthropic uses `cache_control: {"type": "ephemeral"}`, OpenAI/Gemini use automatic implicit caching (flag ignored).
